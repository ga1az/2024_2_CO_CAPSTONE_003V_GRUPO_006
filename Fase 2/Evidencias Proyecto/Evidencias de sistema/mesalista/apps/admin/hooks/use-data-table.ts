...
export function useDataTable<TData>({
  ...
  // Use the module-specific schema
  const search = createSearchParamsSchema(defaultSort).parse(
  ...
  React.useEffect(() => {
    ...
    const newParamsObject: Record<string, any> = {
      page: 1
    }

    // Initialize an array to hold all filters
    const allFilters: string[] = []

    // Handle debounced searchable column filters
    for (const column of debouncedSearchableColumnFilters) {
      if (typeof column.value === 'string' && column.value !== '') {
        allFilters.push(`${column.id}:like:${column.value}%`)
      }
    }
    ...
    for (const column of filterableColumnFilters) {
      if (Array.isArray(column.value) && column.value.length > 0) {
        allFilters.push(`${column.id}:eq:${column.value.join(',')}`)
      }
    }

    // Add filters to newParamsObject if there are any
    if (allFilters.length > 0) {
      newParamsObject.filters = allFilters.join(',')
    } else {
      newParamsObject.filters = null
    }

    // Check for removed filters
    const currentFilters = searchParams.get('filters')?.split(',') || []
    const removedFilters = currentFilters.filter((filter) => {
      const [columnId] = filter.split(':')
      return !allFilters.some((newFilter) =>
        newFilter.startsWith(columnId + ':')
      )
    })

    if (removedFilters.length > 0) {
      newParamsObject.filters =
        allFilters.length > 0 ? newParamsObject.filters : null
    }

    // Remove deleted values for non-filter params
