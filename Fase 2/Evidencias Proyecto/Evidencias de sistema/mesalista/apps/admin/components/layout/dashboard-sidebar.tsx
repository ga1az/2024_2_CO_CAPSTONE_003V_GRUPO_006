  ...
  useEffect(() => {
    const filtered = links.filter((link) => {
      if (link.authorizeOnly) {
        return link.authorizeOnly === user?.role
      }
      return true
    })
        ...
        <ScrollArea className="h-full overflow-y-auto border-r">
          ...
          >
            <div className="flex h-full max-h-screen flex-1 flex-col gap-2">
              ...
              <Separator orientation="horizontal" className="h-px bg-muted" />
              <div
                className={cn(
                  'flex w-full justify-center p-4',
                  !isSidebarExpanded && 'flex-col items-center'
                )}
              >
                <ModeToggle compact={!isSidebarExpanded} />
              </div>
            </div>
          </aside>
        </ScrollArea>
    ...
    return (
      <Sheet open={open} onOpenChange={setOpen}>
        ...
        <SheetContent
          ...
          aria-describedby="SheetTitle"
        >
          <SheetTitle className="hidden">MesaLista</SheetTitle>
          <ScrollArea className="h-full overflow-y-auto">
            <div className="flex h-screen flex-col">
              <nav className="flex flex-1 flex-col gap-y-8 p-6 text-lg font-medium">
                ...
                >
                  <Icons.products className="size-6" />
